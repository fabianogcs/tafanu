...
export function MenuSection({
  ...
  return (
    // 🚀 UX FIX: min-h-[600px] segura o "tranco" da tela para ela não pular!
    <div className="bg-white rounded-[2.5rem] p-6 md:p-10 shadow-sm border border-slate-200 min-h-[600px] flex flex-col transition-all duration-500">
      ...
      <div className="grid grid-cols-3 gap-2 p-1.5 bg-slate-100 rounded-2xl mb-6 shrink-0">
        <button
          type="button"
      ...
      <div className="flex-1 flex flex-col">
        {(menuMode === "DIGITAL" || menuMode === "AGENDA") && (
          <div className="flex bg-slate-100 p-1.5 rounded-2xl mb-8 border border-slate-200 w-full max-w-sm mx-auto shadow-inner shrink-0">
            <button
        ...
        {(menuMode === "DIGITAL" || menuMode === "AGENDA") &&
          isExternalLink && (
            <div className="bg-indigo-50 border border-indigo-100 rounded-[2rem] p-6 md:p-8 animate-in fade-in zoom-in duration-300 mb-8 shrink-0">
        ...
        {menuMode === "AGENDA" && !isExternalLink && (
          <div className="mb-6 bg-emerald-50 border border-emerald-200 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 animate-in fade-in zoom-in duration-300 shadow-sm shrink-0">
            ...
            <div className="flex-1">
              ...
              <p className="text-[10px] font-medium text-emerald-700 leading-relaxed max-w-2xl">
                ...
                <br />
                2. Defina logo abaixo o{" "}
                <strong>Tempo de cada atendimento</strong> e a{" "}
        ...
        {menuMode === "AGENDA" &&
          ...
          agendaConfig &&
          setAgendaConfig && (
            <div className="mb-8 bg-white border border-slate-200 rounded-[2rem] p-6 md:p-8 shadow-sm animate-in fade-in slide-in-from-top-4 shrink-0">
              <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 mt-1">
                    Configure os dias e intervalos exatos que você atende.
                  </p>
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      return (
                        <div
                          key={index}
                          className={`flex flex-col p-4 md:p-5 rounded-2xl border transition-all ${
                            dayData.isClosed
                        ...
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (idx >= 0) {
                                    ...
                                    // Se abriu, reseta as exceções desse dia
                                    if (e.target.checked)
                                      newHours[idx].exceptions = [];
                                  }
                                  setAgendaConfig({
                                    ...agendaConfig,
                                    hours: newHours,
                            ...
                            <div
                              className={`flex flex-1 sm:justify-end items-center gap-3 ${
                                dayData.isClosed ? "pointer-events-none" : ""
                              }`}
                            >
                              <input
                                type="time"
                          ...
                          {/* 🚀 O GERENCIADOR DE EXCEÇÕES (Bloqueio Dinâmico) */}
                          {!dayData.isClosed && (
                            <div className="mt-4 pt-4 border-t border-indigo-50">
                              <p className="text-[9px] font-black uppercase text-indigo-400 tracking-widest mb-3">
                                Bloquear Horários Específicos
                              </p>
                              <div className="flex flex-wrap gap-2">
                                {(() => {
                                  // Lógica para gerar os slots com base na abertura e fechamento
                                  const slots = [];
                                  const [oH, oM] = dayData.openTime
                                    .split(":")
                                    .map(Number);
                                  const [cH, cM] = dayData.closeTime
                                    .split(":")
                                    .map(Number);
                                  let currentMinutes = oH * 60 + oM;
                                  const endMinutes = cH * 60 + cM;
                                  const duration = agendaConfig.duration || 30;
                                    ...
                                    return (
                                      <button
                                        ...
                                        onClick={() => {
                                          ...
                                          if (idx >= 0) {
                                            let currentEx =
                                              newHours[idx].exceptions || [];
                                            if (isBlocked) {
                                              currentEx = currentEx.filter(
                                                (t: string) => t !== slotTime,
                                              );
                                            } else {
                                              currentEx.push(slotTime);
                                            }
                                            newHours[idx].exceptions =
                                              currentEx;
                                            setAgendaConfig({
                                        ...
                                        className={`px-3 py-1.5 rounded-md text-[10px] font-bold border transition-colors ${
                                          isBlocked
                                            ? "bg-rose-100 text-rose-600 border-rose-200 line-through"
        ...
        {/* 🚀 ITENS CADASTRADOS (MOSTRA SE FOR DIGITAL *OU* AGENDA E INTERNO) */}
        {(menuMode === "DIGITAL" || menuMode === "AGENDA") &&
          !isExternalLink && (
                      ...
                      <div className="flex-1 w-full space-y-2">
                        ...
                        <div className="w-full mt-2 pt-3 border-t border-slate-200/60">
                          <label className="text-[9px] font-black uppercase text-slate-400 flex items-center gap-1.5 mb-2">
                            <Plus size={12} /> Adicionais / Complementos
                            (Opcional)
                          </label>
